...
const bookingCode = {
    ...
    getCodeInformation(code) {
        ...
        xhr.open("POST", '/manager/bookingCode/getCode');
    ...
    renderRoomsTypes(data) {
        const _this = this;
		function renderRoomTypeStatus(roomType) {
			if (roomType.status == 1) {
				return `<td style="color: var(--done-button-color);">Normal</td>`
			} else {
				return `<td style="color: var(--cancel-button-color);">${_this.uppercaseFirstLetter(roomType.statusDesc)}</td>`
			}
		}
