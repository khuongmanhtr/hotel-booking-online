<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.0/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-KyZXEAg3QhqLMpG8r+8fhAXLRk2vvoC2f3B09zVXn8CA5QIVfZOJ3BCsw2P0p/We" crossorigin="anonymous">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Roboto:ital,wght@0,100;0,300;0,400;0,500;0,700;1,300;1,400&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css" integrity="sha512-1ycn6IcaQQ40/MKBW2W4Rhis/DbILU74C1vSrLJxCq57o941Ym01SwNsOMqvEBFlcgUa6xLiPY/NS5R+E6ztJQ==" crossorigin="anonymous" referrerpolicy="no-referrer" />
    <link rel="icon" type="image/png" href="/assets/image/icon/hotel.png"/>
    <link rel="stylesheet" href="/assets/css/base.css"/>
    <link rel="stylesheet" href="/assets/css/staff/header_footer.css"/>
    <link rel="stylesheet" href="/assets/css/staff/staff-base.css"/>
    <title>Home | Room - Hotel Management</title>
</head>
<body>
    <!-- Header -->
    <c:import url="../staff/staff-component/header.jsp"/>

    <div class="management">
        <div class="grid">
            <div class="grid-row">
                <div class="grid-col grid-col-2">
                    <c:import url="manager-component/navigation-box.jsp"/>
                </div>
                <div class="grid-col grid-col-10 ">
                    <div class="management__dashboard">
                        <div class="management__dashboard-upper">
                            <ul class="management__dashboard-heading-list">
                                <li class="management__dashboard-heading-item">
                                    <a class="square-btn management__dashboard-heading-link active">All Room</a>
                                </li>
                                <li class="management__dashboard-heading-item">
                                    <a href="/manager/room/allRoomType" class="square-btn management__dashboard-heading-link">All Room Type</a>
                                </li>
                            </ul>
                            <div class="management__dashboard-monitor">
                                <div class="management__room-statistics">
                                    <h3 class="management__room-statistics-heading">Room Statistics</h3>
                                    <table class="table-minimal-type management__room-statistics-table">
                                        <thead>
                                            <tr>
                                                <th>room type</th>
                                                <th>total room quantity</th>
                                                <th>maximum usage capacity</th>
                                                <th>being used</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <c:forEach items="${allRoomTypeInfo}" var="rt">
                                                <tr>
                                                    <td>${rt.key}</td>
                                                    <td>${rt.value.get("totalQuantity")}</td>
                                                    <td>${rt.value.get("maximumUsageCapacity")}</td>
                                                    <td>${rt.value.get("beingUsed")}</td>
                                                </tr>
                                            </c:forEach>
                                        </tbody>
                                    </table>
                                </div>
                                <div class="management__room-rooms">
                                    <div class="management__room-rooms-wrapper">
                                        <input type="hidden" data-csrf-token="1" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                                        <h3 class="management__room-rooms-heading">All room</h3>
                                        <div class="pill-btn blue-type-btn management__room-rooms-new">
                                            <i class="fas fa-plus"></i>
                                            Add New Room
                                        </div>
                                    </div>
                                    <div class="management__room-selection-frame">
                                        <div class="management__room-annotations">
                                            <div class="management__room-annotations-status">
                                                <div class="grid-row">
                                                    <div class="grid-col grid-col-6">
                                                        <div class="management__room-annotations-item">
                                                            <span class="management__room-annotations-color usable"></span>
                                                            <span class="management__room-annotations-desc">Available</span>
                                                        </div>
                                                    </div>
                                                    <div class="grid-col grid-col-6">
                                                        <div class="management__room-annotations-item">
                                                            <span class="management__room-annotations-color being-used"></span>
                                                            <span class="management__room-annotations-desc">Being Used</span>
                                                        </div>
                                                    </div>
                                                    <div class="grid-col grid-col-6">
                                                        <div class="management__room-annotations-item">
                                                            <span class="management__room-annotations-color not-available "></span>
                                                            <span class="management__room-annotations-desc">Not Available</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

                                        <div class="management__room-all">
                                            <div class="grid-row">
                                                <c:forEach items="${allRoom}" var="room">
                                                    <c:choose>
                                                        <c:when test="${room.currentStatus == 'being used'}">
                                                            <div class="grid-col grid-col-1-2 ">
                                                                <div class="management__room-all-detail management__room-view--being-used">
                                                                    <div class="management__room-all-name">${room.roomName}</div>
                                                                    <div class="management__room-all-type">${room.typeName}</div>
                                                                </div>
                                                            </div>
                                                        </c:when>
                                                        <c:when test="${room.currentStatus == 'available'}">
                                                            <div class="grid-col grid-col-1-2 ">
                                                                <div class="management__room-all-detail management__room-view--available">
                                                                    <div class="management__room-all-name">${room.roomName}</div>
                                                                    <div class="management__room-all-type">${room.typeName}</div>
                                                                </div>
                                                            </div>
                                                        </c:when>
                                                        <c:when test="${room.currentStatus == 'not available'}">
                                                            <div class="grid-col grid-col-1-2 ">
                                                                <div class="management__room-all-detail management__room-view--not-available">
                                                                    <div class="management__room-all-name">${room.roomName}</div>
                                                                    <div class="management__room-all-type">${room.typeName}</div>
                                                                </div>
                                                            </div>
                                                        </c:when>
                                                    </c:choose>
                                                </c:forEach>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div id="modal-segment"></div>

    <c:import url="../staff/staff-component/footer.jsp"/>

    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.0/dist/js/bootstrap.min.js" integrity="sha384-cn7l7gDp0eyniUwwAZgrzD06kc/tftFf19TOAs2zVinnD/C7E91j9yyk5//jjpt/" crossorigin="anonymous"></script>
    <script src="/assets/js/staff/manager-room.js"></script>
</body>
</html>
