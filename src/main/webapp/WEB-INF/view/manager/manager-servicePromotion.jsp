...
<head>
    ...
    <title>Home | Service Promotion Code - Hotel Management</title>
</head>
<body>
    <!-- Header -->
                        ...
                        <div class="management__dashboard-upper">
                            <ul class="management__dashboard-heading-list">
                                <li class="management__dashboard-heading-item">
                                    <a href="/manager/promoCode/room" class="square-btn management__dashboard-heading-link">Room Promotion Code</a>
                                </li>
                                <li class="management__dashboard-heading-item">
                                    <a class="square-btn management__dashboard-heading-link active">Service Promotion Code</a>
                                </li>
                            </ul>
                            <div class="management__dashboard-monitor">
                                <input type="hidden" data-csrf-token="1" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                                <form action="/manager/promoCode/service" method="get" class="management__dashboard-search">
                                    <div class="management__dashboard-search-form">
                                        <label class="management__dashboard-search-title">Filter By:</label>
                                        <select class="management__dashboard-search-option">
                                ...
                                <div class="table-background-layer">
                                    <table class="table-foreground-layer management__service-promotion-table">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            <c:if test="${promoCodeList.size() > 0}">
                                                <c:forEach items="${promoCodeList}" var="code">
                                                    <tr>
                                                        <td>${code.servicePromoCode}</td>
                                                        <td>${code.description}</td>
                                                        <c:choose>
                                                            <c:when test="${code.isUsed == '1'}">
                                                                <td>Yes</td>
                                                            </c:when>
                                                            <c:when test="${code.isUsed == '0'}">
                                                                <td>No</td>
                                                            </c:when>
                                                        </c:choose>
                                                        <c:choose>
                                                            <c:when test="${code.currentStatus == 'available'}">
                                                                <td>
                                                                    <span class="management__service-promo-status--available">${code.currentStatus}</span>
                                                                </td>
                                                            </c:when>
                                                            <c:when test="${code.currentStatus == 'being used'}">
                                                                <td>
                                                                    <span class="management__service-promo-status--being-used">${code.currentStatus}</span>
                                                                </td>
                                                            </c:when>
                                                            <c:when test="${code.currentStatus == 'expired'}">
                                                                <td>
                                                                    <span class="management__service-promo-status--expired">${code.currentStatus}</span>
                                                                </td>
                                                            </c:when>
                                                        </c:choose>
                                                        <td>
                                                            <div class="management__room-type-table-btns">
                                                                <span class="pill-btn blue-type-btn management__service-promotion-btn">Details</span>
                                                            </div>
                                                        </td>
                                                        <input type="hidden" name="servicePromoId" value="${code.servicePromoId}"/>
                                                    </tr>
                                                </c:forEach>
                                            </c:if>
                                        </tbody>
                                    </table>
                                    <c:if test="${promoCodeList.size() == 0 || promoCodeList == null}">
                                    ...
                                    <c:when test="${paginationList.size() > 0}">
                                        <div class="pagination__prev">
                                            ...
                                            <c:choose>
                                                ...
                                                <c:otherwise>
                                                    <a href="/manager/promoCode/${directory}?page=${pageIndex - 1}&searchValue=${searchValue}&filter=${filter}" class="pagination__prev-link">
                                                        <i class="fas fa-chevron-left"></i>
                                                    </a>
                                                </c:otherwise>
                                            </c:choose>
                                        ...
                                        <ul class="pagination__page">
                                            <c:forEach items="${paginationList}" var="pagination" varStatus="status">
                                                <li class="pagination__page-number">
                                                    ...
                                                    <c:if test="${indexOfPageIndex != status.index}">
                                                        <a href="/manager/promoCode/${directory}?page=${pagination}&searchValue=${searchValue}&filter=${filter}" class="pagination__page-link">${pagination}</a>
                                                    </c:if>
                                                </li>
                                            </c:forEach>
                                        </ul>
                                        ...
                                        <div class="pagination__next">
                                            <c:choose>
                                                ...
                                                <c:otherwise>
                                                    <a href="/manager/promoCode/${directory}?page=${pageIndex + 1}&searchValue=${searchValue}&filter=${filter}" class="pagination__next-link">
                                                        <i class="fas fa-chevron-right"></i>
                                                    </a>
                                                </c:otherwise>
                                            </c:choose>
    ...
    <script src="/assets/js/staff/manager-servicePromotion.js"></script>
</body>
</html>
