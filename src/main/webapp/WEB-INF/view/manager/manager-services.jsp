...
<body>
                        ...
                        <div class="management__dashboard-upper">
                            <ul class="management__dashboard-heading-list">
                                <li class="management__dashboard-heading-item">
                                    <a href="#" class="square-btn management__dashboard-heading-link active">All Service</a>
                                </li>
                            </ul>
                            <div class="management__dashboard-monitor">
                                <input type="hidden" data-csrf-token="1" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                                <form action="/manager/service" method="get" class="management__dashboard-search">
                                    <div class="management__dashboard-search-form">
                                        <label class="management__dashboard-search-title">Filter:</label>
                                ...
                                <div class="table-background-layer">
                                    <table class="table-foreground-layer management__setup-service-table">
                                        <thead>
                                            <tr>
                                                <th>service name</th>
                                                <th>price (vnd)</th>
                                            ...
                                            <c:if test="${serviceList.size() > 0}">
                                                <c:forEach items="${serviceList}" var="service">
                                                    <tr>
                                                        ...
                                                        <c:choose>
                                                            <c:when test="${service.currentStatus == 'editable'}">
                                                                <td>
                                                            ...
                                                            <c:when test="${service.currentStatus == 'uneditable'}">
                                                                <td>
                                                                    <div class="management__room-type-table-btns">
                                    ...
                                    <c:if test="${serviceList.size() == 0 || serviceList == null}">
                                    ...
                                    <c:when test="${paginationList.size() > 0}">
                                        <div class="pagination__prev">
                                            ...
                                            <c:choose>
                                                ...
                                                <c:otherwise>
                                                    <a href="/manager/service?page=${pageIndex - 1}&searchValue=${searchValue}&filter=${filter}" class="pagination__prev-link">
                                                        <i class="fas fa-chevron-left"></i>
                                                    </a>
                                                </c:otherwise>
                                            </c:choose>
                                        ...
                                        <ul class="pagination__page">
                                            <c:forEach items="${paginationList}" var="pagination" varStatus="status">
                                                <li class="pagination__page-number">
                                                    ...
                                                    <c:if test="${indexOfPageIndex != status.index}">
                                                        <a href="/manager/service?page=${pagination}&searchValue=${searchValue}&filter=${filter}" class="pagination__page-link">${pagination}</a>
                                                    </c:if>
                                                </li>
                                            </c:forEach>
                                        </ul>
                                        ...
                                        <div class="pagination__next">
                                            <c:choose>
                                                ...
                                                <c:otherwise>
                                                    <a href="/manager/service?page=${pageIndex + 1}&searchValue=${searchValue}&filter=${filter}" class="pagination__next-link">
                                                        <i class="fas fa-chevron-right"></i>
                                                    </a>
                                                </c:otherwise>
                                            </c:choose>
    ...
    <script src="/assets/js/staff/manager-services.js"></script>
</body>
</html>
